...
class SketchTest extends Sketch {
	...
	setup(): void {
		...
		this.palette = this.p.shuffle(
			this.p.random(this.colorScheme).colors.concat()
		)
	...
	draw(): void {
		...
		for (
			let y: number = -yStep;
			y < this.p.height + yStep;
			y += yStep / 2
		) {
			this.p.drawingContext.filter =
				'blur(' +
				this.p.int(
					this.p.map(
						this.p.abs(y - (this.p.height * 3) / 5),
						0,
						this.p.height / 2 + yStep,
						0,
						20
					)
				) +
			...
			const n: number = this.p.map(
				this.p.noise(y / nsy, this.p.frameCount / 300),
				-1,
				1,
				0,
				1
			...
			const gradient: any =
				this.p.random() > 0.5
					? this.p.drawingContext.createLinearGradient(
							0,
							-yStep * 2,
							this.p.width,
							yStep * 2
			...
			for (let x: number = -xStep; x < this.p.width + xStep; x += xStep) {
				y2 =
					this.p.noise(x / nsx, y / nsy, this.p.frameCount / 200) *
					yStep *
					4
	...
	drawTexture(target: p5.Graphics, prob: number, bool: boolean): void {
		for (let i: number = 0; i < this.p.width * this.p.height * prob; i++) {
			...
			target.shearX(
				this.p.random(360 / 4) * (this.p.random() > 0.5 ? -1 : 1)
			)
			target.shearY(
				this.p.random(360 / 4) * (this.p.random() > 0.5 ? -1 : 1)
			)
