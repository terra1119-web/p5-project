...
class SketchTest extends Sketch {
	...
	setup(): void {
		...
		const baseFlow: p5.Vector = p5.Vector.fromAngle(
			this.p.random(this.p.TWO_PI)
		).mult(this.p.random(0.1, 0.8))
		this.forceField = new Array(this.p.width * this.p.height)
			.fill(this.p.width * this.p.height)
			.map(() => [baseFlow.x, baseFlow.y])
		...
		for (let kk: number = 3; kk--; ) {
			...
			for (let t: number = 10; t--; ) {
				const dir: p5.Vector = p5.Vector.fromAngle(
					this.p.random(this.p.TWO_PI)
				)
				dir.mult(
					this.p.random(1) ** 2 * minSideLen * 0.7 + minSideLen * 0.2
				)
	...
	draw(): void {
		...
		while (
			this.movers.length &&
			this.movers[this.movers.length - 1][this.PARTICLE_INDEX.life] >
				this.movers[this.movers.length - 1][
					this.PARTICLE_INDEX.maxLife
				] &&
			maxWhile-- > 0
		) {
			this.movers.pop()
		...
		this.movers = this.movers.map((mover: number[]) => {
			...
			const fadeIn: number = this.p.constrain(
				this.p.map(t, 0, 0.1, 0, 1),
				0,
				1
			)
			const fadeOut: number = this.p.constrain(
				this.p.map(t, 0.1, 1, 1, 0),
				0,
				1
			)
			...
			const _huu: number = this.p.abs(
				this.p.createVector(fx, fy).heading()
			)
			const vel: p5.Vector = this.p
				.createVector(fx * moveScale, fy * moveScale)
				.limit(moveLimit)
				.mult(5)
				.mult(i % 2 === 0 ? -1 : 1)
			if (
				...
				this.p.stroke(
					(huu * 30 + 30 + huuhuu * 160 + this.p.frameCount / 100) %
						360,
					glow ? 20 : (90 + ((i * 777.77) % 50)) * (1 - huu) ** 2,
					glow
						? 100
						: (120 - ((i * 333) % 70)) * (1 - huu * 0.85) -
								_huu * 20
				)
			...
			for (let cx: number = 0; cx < cr * 2; cx++) {
				...
				if (
					...
					if (sqrtD < cr ** 2) {
						...
						const forceFieldIndex: number =
							offnx + offny * this.p.width
						if (
							forceFieldIndex >= 0 &&
							forceFieldIndex < this.forceField.length
						) {
							const [fx, fy] = this.forceField[forceFieldIndex]
	...
	relaxFlowField(flowField: number[][], times: number = 1): number[][] {
		...
		for (let t: number = times; t--; ) {
			_flowField = _flowField.map(([fx, fy], index) => {
				...
				const newForce: p5.Vector = this.p
					.createVector(
						fx * 0.5 +
							up[0] * 0.125 +
							right[0] * 0.125 +
