...
class SketchTest extends Sketch {
	...
	setup(s) {
		...
		s.stroke(80, 0, 50, 200)
		s.fill(250, colorInt, colorInt, 220)
		s.ellipseMode(s.CENTER)
		s.smooth()
		drection = s.createVector(0, -s.height)
		for (let i = 0; i < treeMax; i++) {
			const start = s.createVector(s.random(s.width / 4, s.width / 4 * 3), s.height / 1)
			console.log(start)
			const myTree = new tree(start, drection)
			trees[i] = myTree
	...
	draw(s) {
		...
		for (j = 0; j < treeMax; j++) {
			for (let i = 1; i < count[j]; i++) {
				s.strokeWeight(trees[j].twig[trees[j].map[i].x].thickness[trees[j].map[i].y])
				s.line(trees[j].twig[trees[j].map[i].x].location[trees[j].map[i].y - 1].x, trees[j].twig[trees[j].map[i].x].location[trees[j].map[i].y - 1].y,
					trees[j].twig[trees[j].map[i].x].location[trees[j].map[i].y].x, trees[j].twig[trees[j].map[i].x].location[trees[j].map[i].y].y)
			}
		}
		...
		if (size <= 12) size = 12
		for (j = 0; j < treeMax; j++) {
			for (let i = 0; i < trees[j].twig.length; i++) {
				let num = trees[j].twig[i].location.length - 1
				s.ellipse(trees[j].twig[i].location[num].x, trees[j].twig[i].location[num].y, size, size)
			}
		}
	}
...
class branch {
	constructor(s, loc, thic, id, branchIndex) {
		this.s = s
		this.location = this.s.createVector(1)
		this.thickness = [1]
		this.location[0] = this.s.createVector(loc.x, loc.y)
	...
	rotate2D(v, theta) {
		const xTemp = v.x
		v.x = v.x * this.s.cos(theta) - v.y * this.s.sin(theta)
		v.y = xTemp * this.s.sin(theta) + v.y * this.s.cos(theta)
	}
}

class frontier {
	constructor(s, startPoint, direction) {
		this.s = s
		this.location = this.s.createVector(startPoint.x, startPoint.y)
		this.velocity = this.s.createVector(direction.x, direction.y)
		this.thickness = this.s.random(10, 20)
		this.finished = false
	}
	...
	update(factor) {
		if (this.location.x > -10
			& this.location.y > -10
			& this.location.x < this.s.width + 10
			& this.location.y < this.s.height + 10
			& this.thickness > factor) {
			this.velocity.normalize()
			const uncertain = this.s.createVector(this.s.random(-1, 1), this.s.random(-1, 1))
			uncertain.normalize()
			uncertain.mult(0.2)
			this.velocity.mult(0.8)
			this.velocity.add(uncertain)
			this.velocity.mult(this.s.random(8, 15))
...
class tree {
	constructor(s, startPoint, direction) {
		...
		let fr = []
		fr[id] = new frontier(s, startPoint, direction)

		this.twig = new branch(s, 1)
		this.twig[id] = new branch(s, fr[id].location, fr[id].thickness, id, 0)

		this.map = s.createVector(1)
		this.map[0] = s.createVector(id, this.twig[id].location.length - 1)
		...
		while (!growth) {
			...
			for (id = 0; id < fr.length; id++) {
				...
				if (!fr[id].finished) {
					this.twig[id].location = s.append(this.twig[id].location, s.createVector(fr[id].location.x, fr[id].location.y))
					this.twig[id].thickness = s.append(this.twig[id].thickness, fr[id].thickness)
					this.map = s.append(this.map, s.createVector(id, this.twig[id].location.length - 1))

					if (s.random(0, 1) < this.BranchLengthFactor) { // control length of one branch
						...
						if (fr[id].thickness > this.BranchLocationFactor) { // control the number of the locations on all branches, i.e., treeSize.
							fr = s.append(fr, new frontierParent(s, fr[id]))
							this.twig = s.append(this.twig, new branch(s, fr[id].location, fr[id].thickness, id, this.twig[id].location.length - 1))
							let _id = id
							if (_id != 0) for (let _i = 0; _i < 2; _i++) this.twig[this.twig.length - 1].baseIndex[_i] = s.concat(this.twig[this.twig.length - 1].baseIndex[_i], this.twig[_id].baseIndex[_i])
						}
					} // if (random(0, 1) < 0.2)
				} else growthSum += 1
			}
		...
		this.amplitude[0] = s.random(0.006, 0.012)
		this.phaseFactor[0] = s.random(0.6, 1.2)
		this.freq = s.random(0.5, 0.8)
		this.period = 1 / this.freq
		for (let i = 1; i < this.candNum; i++) {
			this.amplitude[i] = this.amplitude[i - 1] * s.random(0.9, 1.4)
			this.phaseFactor[i] = this.phaseFactor[i - 1] * s.random(0.9, 1.4)
		}
	}
	...
	swing(s) {
		for (let i = 0; i < this.candNum; i++) {
			...
			for (let j = 0; j < _num; j++) {
				this.twig[this.candidateIndex[i]].dTheta[j] = this.amplitude[i] * this.dt * s.TWO_PI * this.freq * s.cos(s.TWO_PI * this.freq * this.time - this.phaseFactor[i] * s.PI * j / _num)
			}
		}
